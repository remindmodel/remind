...
All the columns must be present in the `scenario_config_coupled.csv` file, but most of them can be left blank. The required ones are:
   - `scenario`: The name of the scenario, must be unique and match the `scenario` column in REMIND's `scenario_config.csv`
...
Other, optional columns allows you to make a run start only another has finished, set starting conditions, and give you finer control over which data is fed to MAgPIE.
