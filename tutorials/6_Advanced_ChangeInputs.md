...
2. Clone the repo `https://gitlab.pik-potsdam.de/REMIND/preprocessing-remind` to any location on the cluster and edit the `start.R` file by inserting the next revision number. If an old revision number is used, the input data will not be recalculated.
