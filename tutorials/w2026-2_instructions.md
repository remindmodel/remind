...
Use the `scenario_config_ws26.csv` to start a `SSP2-NPi2026` run followed by a `SSP2-PkBdgt650` scenario
...
*Note* In case the `SSP2-PkBdgt650` does not automatically start after the `SSP2-NPi2026` run has finished, edit the `scenario_config_ws26.csv` by putting a `0` in the `start` column of the `SSP2-NPi2026` scenario line and re-run the above command.
...
```
grep -E "(\*\*\* Status: (Normal completion|Execution error\(s\))|LOOPS iteration = )" output/SSP2-PkBudg650_2026-11-13_18.28.33/full.log | tail -2
```

A typical output of the commands looks like

```PowerShell
> Select-String "output\SSP2-PkBudg650_2026-11-13_18.28.33\full.log" -Pattern "(\*\*\* Status: (Normal completion|Execution error\(s\))|LOOPS iteration = )" | Select-Object -Last 2

output\SSP2-PkBudg650_2026-11-13_18.28.33\full.log:10385:--- LOOPS iteration = 44
output\SSP2-PkBudg650_2026-11-13_18.28.33\full.log:10448:*** Status: Normal completion
```
...
Verify the availability of output folders by running `ls output` in your REMIND directory. In case you are using the prepared output directory, the command shows directories `SSP2-NPi2026_2026-11-14_00.24.00` and `SSP2-PkBudg650_2026-11-13_18.28.33`. If you've computed your own scenarios, the time stamps at the end of the directory name will be different, but they still have the `SSP2-NPi2026` and `SSP2-PkBudg650` prefixes.
...
piamPlotComparison::compareScenarios(
    projectLibrary = "remind2",
    mifScen = c(
        "output/SSP2-NPi2026_2026-11-14_00.24.00/REMIND_generic_SSP2-NPi2026.mif",
        "output/SSP2-PkBudg650_2026-11-13_18.28.33/REMIND_generic_SSP2-PkBudg650.mif"
    ), 
    mifHist =   "output/SSP2-NPi2026_2026-11-14_00.24.00/historical.mif",
...
- Paper: https://egusphere.copernicus.org/preprints/2025/egusphere-2026-2284/ 
...
- download the [validationConfig_workshop26.csv](https://rse.pik-potsdam.de/data/remind/public/validationConfig_workshop26.csv)
...
Common git pitfalls include not having the `workshop2026` branch active. Use `git status` in your REMIND directory to make sure you are on the `workshop2026` branch of the REMIND git repo

```PowerShell
> git status
On branch workshop2026 # <---
Your branch is up to date with 'origin/workshop2026'.
...
git pull --rebase origin workshop2026
...
```PowerShell
Rscript -e "renv::restore(lockfile='renv/archive/ws26_renv.lock'); renv::snapshot()"
```
