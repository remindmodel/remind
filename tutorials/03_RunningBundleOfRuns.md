...
The column `copyFromConfig` column allows to specify a different from which all settings not specified (so with empty cells) are copied. Nested assignment is allowed, so `NDC` can specify `Base` here, and `Policy` can specify `NDC`. Only restriction is that the scenario specified in this cell must be defined in an earlier row.
