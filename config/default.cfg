...
cfg$gms$carbonpriceRegi <- "none"           # def = none
...
cfg$gms$cm_NDC_version            <- "2022_cond"    # def <- "2022_cond", "2022_uncond", "2021_cond", "2021_uncond", "2018_cond", "2018_uncond"
cfg$gms$cm_NDC_divergentScenario  <- 0              # def <- 0 or 1 or 2
...
cfg$gms$c_budgetCO2from2020FFI <- 700    # def <- 700
cfg$gms$c_budgetCO2from2020  <- 1150     # def <- 1150
cfg$gms$c_abtrdy             <- 2010     # def <- 2010
cfg$gms$c_abtcst             <- 1        # def <- 1
cfg$gms$cm_regiExoPrice      <- "off"    # def <- "off"
...
cfg$gms$c_CES_calibration_write_prices       <-   0       # def <-  0, switch to 1 if you want to generate price file, you can use this as new p29_cesdata_price.cs4r price input file
cfg$gms$cm_CES_calibration_default_prices    <-   0.01    # def <-  0.01 lower value if input factors get negative shares (xi), CES prices in the first calibration iteration
cfg$gms$c_CES_calibration_industry_FE_target <-   1
cfg$gms$cm_calibration_string                <-   "off"   # def <- "off" label for your calibration run to keep calibration files with different setups apart (e.g. with low elasticities, high eleasticies)
...
cfg$gms$cm_cooling_shares               <- "dynamic"   # def <- "dynamic"
...
cfg$gms$cm_EDGEtr_scen <- "Mix" # def <- "ConvCase". For calibration runs: "ConvCase", "ConvCaseWise". For standard runs:"ConvCase", "ElecEra", "HydrHype", "ConvCaseWise", "ElecEraWise", "HydrHypeWise"
...
cfg$gms$cm_import_EU <- "off" # def <- "off", e.g. "low_h2", "high_h2", "high_synf" 
...
cfg$gms$cm_noPeFosCCDeu <- 0 # def 0, no suppression of Pe2Se Fossil Carbon Capture in Germany, if 1 then no pe2se fossil CO2 capture in Germany
...
cfg$gms$cm_share_LDV_sales <- "off" # def <- "off"; or list of techs with adj upper/lower bounds to mkt share in LDV (ex. for EV, H2V and ICE: 2030.2050.apCarElT.upper 80, 2030.2050.apCarH2T.upper 90, 2030.2050.apCarPeT.lower 5)
...
# Change learn rate
cfg$gms$cm_learnRate <- "off" # def <- "off"; or list of techs to change learn rate value. (ex. "apcarelt 0.2")
...
cfg$gms$cm_CESMkup_build <- "standard" # def = "standard", applies a markup cost of 200 USD/MWh(el) to heat pumps (feelhpb) and 25 USD/MWh(heat) to district heating (feheb)
...
cfg$gms$cm_CESMkup_ind <- "standard" # def = "standard", applies a markup cost of 0.5 trUSD/TWa (57 USD/MWh(el)) to industry electricity (feeli) 
...
cfg$gms$cm_ariadne_VRECapFac_adj <- "off" # def <- "off", if "on": small increase of VRE capacity factors for wind and solar PV until 2045 in Germany in line with ARIADNE assumptions
...
cfg$gms$c_VREPot_Factor <- "off" # def <- "off"
...
cfg$gms$cm_FEtax_trajectory_abs <- "off" # def <- "off"
...
# (note: don't put values to 0 as this will make the model ignore the switch)
cfg$gms$cm_FEtax_trajectory_rel <- "off" # def <- "off"
...
cfg$gms$cm_VRE_supply_assumptions <- 0 # 0 - default, 1 - optimistic, 2 - sombre, 3 - bleak
...
cfg$gms$cm_INCONV_PENALTY_FESwitch <- "on"    # def = on
...
# cm_NDC_divergentScenario "choose scenario about convergence of CO2eq prices in realization NDC of 45_carbonprice"
...
#c_budgetCO2from2020 "cumulative CO2|Total emissions budget 2020-2100, Gt CO2"
...
############################################################
# cfg$gms$cm_flex_tax: switch to switch flexibility tax on or off
# cfg$gms$cm_PriceDurSlope_elh2: slope of price duration curve for electrolysis (increase means more flexibility subsidy for electrolysis H2)
# cfg$gms$cm_FlexTaxFeedback: switches on feedback of flexibility tax on buildings/industry (electricity price decrease for electrolysis has to be matched by electrictiy price increase in buildings/industry)
# cfg$gms$cm_VRE_supply_assumptions: 
