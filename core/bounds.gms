...
*cb* make sure that grade distribution in early time steps with capacity fixing is close to optimal one assumed for vm_capFac calibration
       vm_capDistr.lo("2015",regi,te,rlf) = 0.90 / p_aux_capacityFactorHistOverREMIND(regi,te) * p_aux_capThisGrade(regi,te,rlf);
       vm_capDistr.lo("2020",regi,te,rlf) = 0.90 / p_aux_capacityFactorHistOverREMIND(regi,te) * p_aux_capThisGrade(regi,te,rlf);
    );
  );
);
