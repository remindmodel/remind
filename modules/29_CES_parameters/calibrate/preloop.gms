...
loop (cesOut2cesIn(in2,in)$(   
                    (NOT (ppf_29(in) AND in_complements(in)) AND ipf_29(in2))
                 OR (in_industry_dyn37(in2) AND ipf(in2))                     ),
...
*** FIXME ===== I N D U S T R Y   S E C T I O N ===============================
file file_parcalc_industry 
     /"parcalc_industry_%c_CES_calibration_iteration%.txt"/;

file_parcalc_industry.lw = 0;
file_parcalc_industry.nr = 2;
file_parcalc_industry.nd = 3;
file_parcalc_industry.nw = 0;
file_parcalc_industry.nz = 0;

*** ----- Reset ue/ppf Prices -------------------------------------------------
$ifthen "%c_CES_calibration_prices%" == "derive" !! c_CES_calibration_prices
loop (industry_ue_calibration_target_dyn37(out),
  !! Prices of industry quantity targets are kept at the dY/dV level, instead 
  !! of being set to 1, like the other ipf.
  pm_cesdata(t,regi_dyn29(regi),out,"price")
  = p29_CESderivative(t,regi,"inco",out);

  !! Prices of ppf to the industry quantity targets are adjusted to compensate 
  !! for the up-stream price change.
  pm_cesdata(t,regi_dyn29(regi),ppf(in),"price")$( cesOut2cesIn_below(out,in) )
...
*** ----- Minimum Prices ------------------------------------------------------
pm_cesdata(t,regi_dyn29(regi),in_industry_dyn37(in),"price")
= max(1e-4, pm_cesdata(t,regi,in,"price"));

put file_parcalc_industry;
*** ----- Historic Periods ----------------------------------------------------
put ">>> Historic Periods Calculations <<<" /;
loop ((t_29hist(t),regi_dyn29(regi)),

  !! - [x] derive prices
  put "--- derive prices ---" /;
  loop (in_industry_dyn37(in),
    put pm_cesdata.tn(t,regi,in,"price"), " = ";
    put pm_cesdata(t,regi,in,"price") /;
  );
  put " " /;

  !! - adjust ppf prices for UEs with only ppf inputs
  loop (industry_ue_calibration_target_dyn37(out)$(
                                           sum(cesOut2cesIn(out,ipf), 1) eq 0 ),
    sm_tmp
    ...
    pm_cesdata(t,regi,in,"price")$( cesOut2cesIn(out,in) )
    = pm_cesdata(t,regi,in,"price")
    * sm_tmp;
  );


  !! - [x] replace ppf quantities (from datainput)
  !! - calculate ipf quantities
  put "--- calculate ipf quantities ---" /;
  loop (cesRev2cesIO(counter,ipf(in_industry_dyn37(out)))$(
                                NOT industry_ue_calibration_target_dyn37(out) ),
    pm_cesdata(t,regi,out,"quantity")
    = sum(cesOut2cesIn(out,in),
        pm_cesdata(t,regi,in,"price")
      * pm_cesdata(t,regi,in,"quantity")
  ...
  !! - adjust ipf prices below UE target
  put "--- adjust ipf prices ---" /;
  loop (industry_ue_calibration_target_dyn37(out)$(
                                           !! only if ipf leaves
                                           sum(cesOut2cesIn(out,ipf), 1) ne 0 ),
    put t.tl, "   ", regi.tl, "   ", out.tl /;
    loop (cesOut2cesIn(out,ipf),
      put @3 pm_cesdata.tn(t,regi,ipf,"price"), "   ";
      put pm_cesdata(t,regi,ipf,"price"), " -> ";
    );

    sm_tmp
    = ( pm_cesdata(t,regi,out,"quantity")
      - sum(cesOut2cesIn(out,ppf),
          pm_cesdata(t,regi,ppf,"price")
        * pm_cesdata(t,regi,ppf,"quantity")
        )
      )
    / sum(cesOut2cesIn(out,ipf),
        pm_cesdata(t,regi,ipf,"price")
      * pm_cesdata(t,regi,ipf,"quantity")
      );

    pm_cesdata(t,regi,ipf,"price")$( cesOut2cesIn(out,ipf) )
    = pm_cesdata(t,regi,ipf,"price")
    * sm_tmp;
  ...
  put "--- calculate efficiency parameters ---" /;
  loop ((cesOut2cesIn(out,in),cesOut2cesIn_below("ue_industry",in))$( 
                                 NOT industry_ue_calibration_target_dyn37(in) ),
    put "pm_cesdata('", t.tl, "','", regi.tl, "','", in.tl, "')" /;
    ...
    * pm_cesdata(t,regi,in,"quantity")
    ...
    put "effGr: ", pm_cesdata(t,regi,in,"effGr") /;
    put " " /;
  );
  put " " /;

  !! above UE targets
  put "=== above UE targets ===" /;
  loop (cesRev2cesIO(counter,out)$( 
               sum(cesOut2cesIn(out,industry_ue_calibration_target_dyn37), 1) ),
    put cesRev2cesIO.tn(counter,out) /;
    
$ontext
    if (%c_CES_calibration_iteration% gt 1, !! c_CES_calibration_iteration gt 1
      put "--- adjust prices ---" /;
      loop (cesOut2cesIn(out,in),
         ...
         ** sign(pm_cesdata(t,regi,out,"rho"))
          );

        put t.tl, "   ", regi.tl, "   ", in.tl, "   (";
        put p29_cesIO_load(t,regi,in), " / ";
        put pm_cesdata(t,regi,in,"quantity"), ") ^ ";
        if (sign(pm_cesdata(t,regi,out,"rho")) eq 1, put "1"; else put "-1");
        put " = ", sm_tmp /;
        if (sm_tmp ne 1,
          put "   ", pm_cesdata(t,regi,in,"price"), " -> ";
...
$offtext
    ...
    loop (cesOut2cesIn(out,in),
      ...
      * pm_cesdata(t,regi,in,"quantity")
      ...
      put pm_cesdata.tn(t,regi,in,"xi"),  " = ", pm_cesdata(t,regi,in,"xi") /;
      put pm_cesdata.tn(t,regi,in,"eff"), " = ", pm_cesdata(t,regi,in,"eff") /;
      ...
      put pm_cesdata.tn(t,regi,in,"effGr"), " = ";
...
*** ----- Special Case Secondary Steel ----------------------------------------
!! FIXME replace with proper set
loop ((regi_dyn29(regi),cesOut2cesIn2(out,in2))$( 
                                           sameas(in2,"feel_steel_secondary") ),
  !! - [x] derive prices

  !! - calculate energy quantity trajectory from ue_steel_secondary and 
  !!   efficiency growth
  loop ((t2(t_29hist),cesOut2cesIn(out,ppfen(in)))$( 
                                              ord(t_29hist) eq card(t_29hist) ),
    pm_cesdata(t,regi,in,"quantity")$( NOT t_29hist(t) )
    = pm_cesdata(t,regi,out,"quantity")
      !! specific energy use in last historic period
    * (pm_cesdata(t2,regi,in,"quantity") / pm_cesdata(t2,regi,out,"quantity"))
      !! efficiency growth trajectory
    * ((1 + pm_ue_eff_target(out)) ** (pm_ttot_val(t2) - t.val));
  );

  !! constant quantities after 2100
  loop (cesOut2cesIn(out,ppf(in)),
    pm_cesdata(t,regi,in,"quantity")$( t.val gt 2100 )
    = pm_cesdata("2100",regi,in,"quantity");
  );
  
  !! adjust  prices to meet Euler identity
  loop (t$( NOT t_29hist(t) ),
    sm_tmp
    = pm_cesdata(t,regi,out,"quantity")
    ...
    pm_cesdata(t,regi,ppf(in),"price")$( cesOut2cesIn_below(out,in) )
    = pm_cesdata(t,regi,in,"price")
    * sm_tmp;
  );
  
  !! - [x] replace ppf quantites
  !! - calculate efficiency parameters
  loop (cesOut2cesIn(out,in),
    pm_cesdata(t,regi,in,"xi")
    = pm_cesdata(t,regi,in,"price")
...
*** ----- Remaining Cases Efficiency Growth Targets ---------------------------
put ">>> Efficiency Growth Targets <<<" /;
!! - [x] derive prices

!! - keep ppf quantities
!!   - estimate from UE in first iteration w/ new structure
if (    %c_CES_calibration_iteration% eq 1 
    AND %c_CES_calibration_new_structure% eq 1,
  loop ((t2(t_29hist),ppfen_industry_dyn37(in))$( 
                                        ord(t_29hist) eq card(t_29hist) 
                                        !! FIXME replace with proper set
                                    AND NOT sameas(in,"feel_steel_secondary") ),

    pm_cesdata(t,regi_dyn29(regi),in,"quantity")$( NOT t_29hist(t) )
    = pm_cesdata(t2,regi,in,"quantity")
    * sum(cesOut2cesIn_below(industry_ue_calibration_target_dyn37(out),in),
        pm_cesdata(t,regi,out,"quantity")
      / pm_cesdata(t2,regi,out,"quantity")
      * ((1 + pm_ue_eff_target(out)) ** (pm_ttot_val(t2) - t.val))
      );
  );
!!   - load from gdx otherwise
else
                                       !! FIXME replace with proper set
  loop (ppfen_industry_dyn37(in)$( NOT sameas(in,"feel_steel_secondary") ),
    pm_cesdata(t,regi_dyn29(regi),in,"quantity")$( NOT t_29hist(t) )
    = p29_cesIO_load(t,regi,in);
  );
);

!!   - constant quantites after 2100
loop (in_industry_dyn37(ppf(in))$( 
                              NOT cesOut2cesIn_below("ue_steel_secondary",in) ),
  pm_cesdata(t,regi_dyn29(regi),in,"quantity")$( t.val gt 2100 )
  = pm_cesdata("2100",regi,in,"quantity");
);

!! fill efficiency parameters for ppfen and ipf, except ipf directly below UE
loop ((t2(t_29hist),cesOut2cesIn(in_industry_dyn37(out),in))$( 
                              ord(t_29hist) eq card(t_29hist)
                          AND (ppfen(in) OR ipf(in))
                          AND NOT industry_ue_calibration_target_dyn37(out)
                          AND NOT cesOut2cesIn_below("ue_steel_secondary",in) ),

  pm_cesdata(t,regi_dyn29(regi),in,"xi")$( NOT t_29hist(t) )
  = pm_cesdata(t2,regi,in,"xi");

  pm_cesdata(t,regi_dyn29(regi),in,"eff")$( NOT t_29hist(t) )
...
loop ((t0,cesOut2cesIn(in_industry_dyn37(out),in))$( 
                              (ppfen(in) OR ipf(in))
                          AND NOT industry_ue_calibration_target_dyn37(out)
                          AND NOT cesOut2cesIn_below("ue_steel_secondary",in) ),

  !! transfer time-variant components to effGr
  pm_cesdata(t,regi_dyn29(regi),in,"effGr")$( NOT t_29hist(t) )
  = (pm_cesdata(t,regi,in,"eff") / pm_cesdata(t0,regi,in,"eff"))
  * (pm_cesdata(t,regi,in,"xi")  / pm_cesdata(t0,regi,in,"xi"))
 ** (1 / pm_cesdata(t,regi,out,"rho"));

  !! keep xi and eff fixed
  pm_cesdata(t,regi_dyn29(regi),in,"xi")$( NOT t_29hist(t) )
  = pm_cesdata(t0,regi,in,"xi");

  pm_cesdata(t,regi_dyn29(regi),in,"eff")$( NOT t_29hist(t) )
  = pm_cesdata(t0,regi,in,"eff");
);

!! - calculate ipf from input quantities and efficiency parameters
loop ((cesRev2cesIO(counter,ipf(out)),
       cesOut2cesIn_below(industry_ue_calibration_target_dyn37,out))$(
                             NOT cesOut2cesIn_below("ue_steel_secondary",out) ),

  pm_cesdata(t,regi_dyn29(regi),out,"quantity")$( NOT t_29hist(t) )
  = sum(cesOut2cesIn2(out,in),
      pm_cesdata(t,regi,in,"xi")
    * ( pm_cesdata(t,regi,in,"eff")
      * pm_cesdata(t,regi,in,"effGr")
      * pm_cesdata(t,regi,in,"quantity")
...
!! - adjust ipf quantity directly below UE to match energy efficiency trajectory
loop (cesOut2cesIn(industry_ue_calibration_target_dyn37(out),ipf(in))$(
                                         NOT sameas(out,"ue_steel_secondary") ),
  pm_cesdata(t,regi_dyn29(regi),in,"quantity")$( NOT t_29hist(t) )
  = pm_cesdata(t,regi,in,"quantity")
    !! specific energy use in t
  / ( sum(cesOut2cesIn_below(out,ppfen), pm_cesdata(t,regi,ppfen,"quantity"))
    / pm_cesdata(t,regi,out,"quantity")
    )
  * sum(t2(t_29hist)$( ord(t_29hist) eq card(t_29hist) ),
      !! specific energy use in last historic t2
      sum(cesOut2cesIn_below(out,ppfen), pm_cesdata(t2,regi,ppfen,"quantity"))
    / pm_cesdata(t2,regi,out,"quantity")
      !! enery efficiency growth trajectory
    * ((1 + pm_ue_eff_target(out)) ** (pm_ttot_val(t2) - t.val))
    );
);

!! - adjust prices directly below UE to satisfy Euler identity
loop ((t,regi_dyn29(regi),industry_ue_calibration_target_dyn37(out))$(
                                         NOT t_29hist(t) 
                                     AND NOT sameas(out,"ue_steel_secondary") ),
  sm_tmp
    !! bottom-up value
  = sum(cesOut2cesIn(out,in),
      pm_cesdata(t,regi,in,"price")
...
!! - calculate efficiency parameters directly below UE from quantities and 
!!   prices
loop (cesOut2cesIn(industry_ue_calibration_target_dyn37(out),in)$(
                                         NOT sameas(out,"ue_steel_secondary") ),
  pm_cesdata(t,regi_dyn29(regi),in,"xi")$( NOT t_29hist(t) )
  = pm_cesdata(t,regi,in,"price")
  * pm_cesdata(t,regi,in,"quantity")
...
*** ----- Efficiency Parameters Above Industry UE Targets ---------------------
put logfile, ">>> UE quantities <<<" /;
$ontext
loop (cesRev2cesIO(counter,out)$(
         sum(cesOut2cesIn2(out,industry_ue_calibration_target_dyn37(in2)), 1) ),
  loop ((t,regi_dyn29(regi))$( NOT t_29hist(t) ),
    !! Adjust UE prices in order to meet target quantities
    !! - if result quantity (last iteration) is larger than the target,
    !!   the result quantity (next iteration) should go down, therefore
    !! - efficiency should go up (down) for rho > 0 (rho < 0), therefore
    !! - xi should go up (down), as eff is fixed by quantities, therefore
    !! - adjusted prices should go up (down), therefore
    !! - the scaling parameter should go up (down), therefore
    !! - the numerator should be larger (smaller) than the denominator, 
    !!   therefore
    !! - the result quantity is in the numerator, the target quantity is in the
    !!   denominator, and the quotient is raised to the power of sign(rho),
    !!   inverting it when rho < 0

    !! exclude first iteration, since no data from previous iteration is 
    !! available
    if (%c_CES_calibration_iteration% gt 1, !! c_CES_calibration_iteration gt 1
      loop (cesOut2cesIn(out,in),

put pm_cesdata.tn(t,regi,in,"price"), ":  ";
put pm_cesdata(t,regi,in,"price"), " * (", p29_cesIO_load(t,regi,in), " / ";
put pm_cesdata(t,regi,in,"quantity"), ") ^ ";
if (sign(pm_cesdata(t,regi,out,"rho")) eq 1, put "1"; else put "-1");
put " = ";
...
!! Calculate ipf quantities
loop (cesRev2cesIO(counter,out)$( 
           sum(cesOut2cesIn(out,industry_ue_calibration_target_dyn37(in)), 1) ),
...
put logfile, ">>> UE efficiencies <<<" /;
loop ((cesRev2cesIO(counter,out),cesOut2cesIn(out,in))$( 
         sum(cesOut2cesIn2(out,industry_ue_calibration_target_dyn37(in2)), 1) ),
  !! calculate efficiency parameters from quantities (and prices)
  pm_cesdata(t,regi_dyn29(regi),in,"xi")$( NOT t_29hist(t) )
  = pm_cesdata(t,regi,in,"price")
  * pm_cesdata(t,regi,in,"quantity")
  / pm_cesdata(t,regi,out,"quantity");
...
loop ((t_29(t),t0,regi_dyn29(regi))$( NOT t_29hist(t) ),
  put pm_cesdata.tn(t,regi,in,"xi");
  put @54 " = ", pm_cesdata(t,regi,in,"price");
  put " * ", pm_cesdata(t,regi,in,"quantity");
  put " / ", pm_cesdata(t,regi,out,"quantity");
  put " = ", pm_cesdata(t,regi,in,"xi") /;

  put pm_cesdata.tn(t,regi,in,"eff");
  put @54 " = ", pm_cesdata(t,regi,out,"quantity");
  put " / ", pm_cesdata(t,regi,in,"quantity");
  put " = ", pm_cesdata(t,regi,in,"eff") /;

  put pm_cesdata.tn(t,regi,in,"effGr"), @54 " = ";
  put ( (pm_cesdata(t,regi,in,"eff") / pm_cesdata(t0,regi,in,"eff"))
      * (pm_cesdata(t,regi,in,"xi")  / pm_cesdata(t0,regi,in,"xi"))
     ** (1 / pm_cesdata(t,regi,out,"rho"))
      ) /;
);
...
*** ----- Constant Efficiency After 2100 --------------------------------------
loop (t_29_last,
  pm_cesdata(t,regi_dyn29(regi),in_industry_dyn37(in),"effgr")$(
                               t.val gt pm_ttot_val(t_29_last)
                          AND  NOT cesOut2cesIn_below("ue_steel_secondary",in) )
...
pm_cesdata(t_29,regi_dyn29, in_29, "effGr")$( NOT in_industry_dyn37(in_29) ) 
  = 1;
...
loop  ((cesRev2cesIO(counter,ipf_29(out)),ces_29(out,in))$( 
                                                    NOT sameas(out,"inco") 
                                 AND NOT cesOut2cesIn_below("ue_industry",in) ),
...
$else.prices_beyond
  ...
  pm_cesdata(t,regi,ipf_beyond_29,"price")$( NOT in_industry_dyn37(ipf_beyond_29) ) = 1;
...
$endif.repEsubs
  ...
  loop ((t_29hist(t),regi_dyn29(regi),root_beyond_calib_29(out))$( 
    ...
    loop (cesOut2cesIn_below(out,ppf(in))$( NOT in_industry_dyn37(in) ),
      pm_cesdata(t,regi,in,"price")
      = pm_cesdata(t,regi,in,"price") 
      / sm_tmp;
  ...
  loop ((t_29hist(t_29),cesOut2cesIn(out,in_beyond_calib_29_excludeRoot(in)))$(
                                     NOT cesOut2cesIn_below("ue_industry",in) ),
    if (NOT ipf_putty(out),
      pm_cesdata(t_29,regi_dyn29,in,"xi")
      = pm_cesdata(t_29,regi_dyn29,in,"price")
      * pm_cesdata(t_29,regi_dyn29,in,"quantity")
...
loop ((t0,in_beyond_calib_29_excludeRoot)$( NOT in_industry_dyn37(in_beyond_calib_29_excludeRoot) ),
...
loop ((t_29_last,in)$( NOT cesOut2cesIn_below("ue_industry",in) ),
