...
pm_MportsRegi(t,regi,regi2,tradeSe)$(pm_seTradeCapacity(t,regi2,regi,tradeSe)) =
  vm_Mport.l(t,regi,tradeSe)*
 (pm_seTradeCapacity(t,regi2,regi,tradeSe)/sum(regi3,pm_seTradeCapacity(t,regi3,regi,tradeSe)))
;

*** Xports quantitites as a result of Mports
pm_XportsRegi(t,regi,regi2,tradeSe) = pm_MportsRegi(t,regi2,regi,tradeSe);
...
pm_MPortsPrice(t,regi,tradeSe)$(sum(regi2,pm_XportsRegi(t,regi2,regi,tradeSe)) gt 0) =
  sum(regi2$pm_seTradeCapacity(t,regi2,regi,tradeSe),
    pm_XPortsPrice(t,regi2,tradeSe)
    *(
      pm_XportsRegi(t,regi2,regi,tradeSe)
      /
      sum(regi3$pm_seTradeCapacity(t,regi3,regi,tradeSe), pm_XportsRegi(t,regi3,regi,tradeSe))
    )
  )
;

*** Fixing exports for current iteration based on previous iteration trade results
vm_Xport.fx(t,regi,tradeSe) = sum(regi2, pm_XportsRegi(t,regi,regi2,tradeSe));

display  pm_seTradeCapacity, pm_MportsRegi, pm_XportsRegi, pm_MPortsPrice, pm_XPortsPrice, pm_SEPrice;
