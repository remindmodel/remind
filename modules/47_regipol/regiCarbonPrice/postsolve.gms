...
* save p47_slope_beforeTarget to new parameter defined over all _regi
	p47_slope_beforeTarget_allregi(ttot2,all_regi)=p47_slope_beforeTarget(ttot2,ext_regi);
* set p47_slope_beforeTarget_allregi to 1 in target year for regions not specified in cm_regipol_slope_beforeTarget such that they have a straight linear co2 price trajecotry until start year and not two different slopes
	if( NOT p47_slope_beforeTarget_regi(ext_regi),
		p47_slope_beforeTarget_allregi(ttot2,all_regi)=1;
	);
* helper parameter to make following loop shorter
		p47_slope_beforeTarget_timeStep(ttot3) = p47_slope_beforeTarget_allregi(ttot3,all_regi); 
...
*** linear price development with different slope scaled by cm_regipol_slope_beforeTarget from end of first slope until target year
				pm_taxCO2eq(t,all_regi)$(t.val ge ttot3.val AND t.val le ttot2.val)  = pm_taxCO2eq(ttot3,all_regi) + p47_slope_beforeTarget_allregi(ttot3,all_regi) * p47_slope_firstYears(all_regi)*(t.val-ttot3.val); 
