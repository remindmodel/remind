		...
		loop((ttot,target_type,emi_type)$p47_regiCO2ETStarget(ttot,target_type,emi_type),
           if(sameas(target_type,"budget"),
			...
			if(sameas(target_type,"year"),
	            pm_ETSTarget_dev(ETS_mkt) = (p47_emiCurrentETS(ETS_mkt) - p47_regiCO2ETStarget(ttot,target_type,emi_type))/p47_emissionsRefYearETS(ETS_mkt);		 
			);	
...
*** dampen rescale factor with increasing iterations to help convergence if the last two iteration deviations where not in the same direction 
            if((iteration.val gt 3) and (p47_ETSTarget_dev_iter(iteration, ETS_mkt)*p47_ETSTarget_dev_iter(iteration+1, ETS_mkt) < 0),
...
***		pm_ESRTarget_dev(t,regi)$pm_emiTargetES(t,regi) = (v47_emiTargetMkt.l(t,regi,"ES","%cm_emiMktES_type%")-pm_emiTargetES(t,regi))/pm_emiTargetES(t,regi);
		pm_ESRTarget_dev(t,regi)$pm_emiTargetES(t,regi) = (v47_emiTargetMkt.l(t,regi,"ES","%cm_emiMktES_type%")-pm_emiTargetES(t,regi))/(p47_emissionsRefYearESR("2005",regi)/sm_c_2_co2);
...
***			)
			sum(regi2$regi_group("EU27_regi",regi2),
				(p47_emissionsRefYearESR("2005",regi2)/sm_c_2_co2)
			)
		; 
		p47_ESRTarget_dev_iter(iteration, t,regi) = pm_ESRTarget_dev(t,regi);	
...
*** dampen rescale factor with increasing iterations to help convergence if the last two iteration deviations where not in the same direction 
        loop(t$p47_emiRescaleCo2TaxES(t,regi),
			if((iteration.val gt 3) and (p47_ESRTarget_dev_iter(iteration,t,regi)*p47_ESRTarget_dev_iter(iteration-1,t,regi) < 0),
				p47_emiRescaleCo2TaxES(t,regi) =
...
*** dampen rescale factor with increasing iterations to help convergence if the last two iteration deviations where not in the same direction 
	if((iteration.val gt 3) and (p47_regiTarget_dev_iter(iteration,ext_regi,ttot,ttot2)*p47_regiTarget_dev_iter(iteration-1,ext_regi,ttot,ttot2) < 0),
