...
Set pm_NDCyearSet(ttot,all_regi)                   "YES for years whose NDC targets is used";
...
pm_NDCyearSet(t,regi)$(t_NDC_targetYear(t)) = p45_shareTarget(t,regi) >= p45_minRatioOfCoverageToMax * p45_bestNDCcoverage(regi);

$ifThen "%cm_targetDelay%" == "prisma"
*** PRISMA Asymetric rollback
** Requires cm_NDC_version = 2026_cond: copy 2030 and 2035 targets to later years based on region delay, set 2030 and 2035 targets to 0
pm_NDCyearSet(t,regi)$(t.val eq 2030 + p45_delay(regi)) = pm_NDCyearSet("2030",regi);
pm_NDCyearSet(t,regi)$(t.val eq 2035 + p45_delay(regi)) = pm_NDCyearSet("2035",regi);
pm_NDCyearSet("2070","REF") = pm_NDCyearSet("2035","REF");
pm_NDCyearSet("2070","MEA") = pm_NDCyearSet("2035","MEA");
pm_NDCyearSet(t,regi)$(t.val eq 2030) = 0;
pm_NDCyearSet(t,regi)$(t.val eq 2035) = 0;
...
pm_NDCyearSet("2030","USA") = NO;
pm_NDCyearSet("2035","USA") = NO;
$ENDIF



if(p45_useSingleYearCloseTo > 0,
  p45_distanceToOptyear(pm_NDCyearSet(t,regi)) = abs(t.val - p45_useSingleYearCloseTo);
  p45_minDistanceToOptyear(regi) = smin(t$(pm_NDCyearSet(t,regi)), p45_distanceToOptyear(t,regi));
  pm_NDCyearSet(t,regi) = p45_distanceToOptyear(t,regi) = p45_minDistanceToOptyear(regi);
);

*** first and last NDC year as a number
Parameter p45_firstNDCyear(all_regi) "last year with NDC coverage within region [year]";
p45_firstNDCyear(regi) = smin( pm_NDCyearSet(t, regi), t.val );
...
p45_lastNDCyear(regi)  = smax( pm_NDCyearSet(t, regi), t.val );

display pm_NDCyearSet,p45_firstNDCyear,p45_lastNDCyear;
