...
***create problems with regions with no pecoal demand in 2005 in EU11 (ENC,ESW). It would force a capacity that does not exist for them in 2005.
***vm_fuExtr.lo(t,regi,peFos(enty),"1")$(pm_ffPolyCumEx(regi,enty,"max")>1e-6) = 1e-6;
vm_fuExtr.lo(t,regi,peFos(enty),"1")$((pm_ffPolyCumEx(regi,enty,"max")>1e-6) AND (NOT (SAMEAS(regi,"EWN") AND SAMEAS(enty,"pecoal") AND SAMEAS(t,"2005")))) = 1e-6;
