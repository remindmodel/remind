...
p21_taxrevImport0(ttot,regi,tradePe,tax_import_type_21) =  p21_tau_Import(ttot,regi,tradePe,tax_import_type_21)$sameas(tax_import_type_21, "worldPricemarkup") * pm_pvp(ttot,tradePe) / pm_pvp(ttot,"good") * vm_Mport.l(ttot,regi,tradePe)+
  p21_tau_Import(ttot, regi, tradePe, tax_import_type_21)$sameas(tax_import_type_21, "c02taxmarkup") * max(0,(pm_taxCO2eqSum(ttot,regi)- sum(regi2, pm_taxCO2eqSum(ttot,regi2))/(card(regi2)))) * p_cintraw(tradePe) *  vm_Mport.l(ttot,regi,tradePe)+
  p21_tau_Import(ttot, regi, tradePe, tax_import_type_21)$sameas(tax_import_type_21, "avC02taxmarkup") * max(pm_taxCO2eqSum(ttot,regi), sum(regi2, pm_taxCO2eqSum(ttot,regi2))/(card(regi2))) * p_cintraw(tradePe) *  vm_Mport.l(ttot,regi,tradePe);

display v21_taxrevImport.l;
display p21_tau_Import;
display pm_taxCO2eqSum;
display p_cintraw;
display vm_Mport.l;
display p21_taxrevImport0;
