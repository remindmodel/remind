...
if(cm_fetaxscen eq 0, !! no FE tax, constant PE2SE tax
  ...
  elseif cm_fetaxscen eq 2, !! converging FE tax (-2050), constant PE2SE tax
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(ttot.val gt 2005) = p21_tau_fe_tax("2005",regi,sector,entyFe);
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(p21_tax_convergence("2050",regi,entyFe) AND ttot.val gt 2025 AND ttot.val le 2050)
	  = p21_tau_fe_tax("2025",regi,sector,entyFe) 
      + ( p21_tax_convergence("2050",regi,entyFe) - p21_tau_fe_tax("2025",regi,sector,entyFe) ) * ( ( ttot.val - 2025 ) / ( 2050 - 2025 ) );
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(p21_tax_convergence("2050",regi,entyFe) AND ttot.val gt 2050) = p21_tax_convergence("2050",regi,entyFe);
  elseif cm_fetaxscen eq 5, !! rollback FE tax (-2035), no PE2SE tax
    p21_tau_pe2se_tax(tall,regi,te) = 0;
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(ttot.val gt 2005) = p21_tau_fe_tax("2005",regi,sector,entyFe);
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(p21_tax_convergence_rollback("2035",regi,entyFe) AND ttot.val gt 2025 AND ttot.val le 2035 )
	   = p21_tau_fe_tax("2025",regi,sector,entyFe) 
       + ( p21_tax_convergence_rollback("2035",regi,entyFe) - p21_tau_fe_tax("2025",regi,sector,entyFe) ) * ( (ttot.val - 2025) / (2035 - 2025) );
    p21_tau_fe_tax(ttot,regi,sector,entyFe)$(p21_tax_convergence_rollback("2035",regi,entyFe) AND ttot.val gt 2035) = p21_tax_convergence_rollback("2035",regi,entyFe);
);
...
if (cm_fetaxscen eq 0, !! no FE and ResEx sub
  ...
  elseif (cm_fetaxscen eq 1) or (cm_fetaxscen eq 5), !! constant FE and ResEx sub, in case of 5 (i.e. rollback) some reduction in tax levels to avoid absurd results
    ...
    if ( (cm_fetaxscen eq 5),
      p21_tau_fe_sub(ttot,regi,sector,entyFe)$(p21_sub_convergence_rollback("2035",regi,sector,entyFe) gt 0 AND ttot.val gt 2025 AND ttot.val le 2035 )
   	      = p21_tau_fe_sub("2025",regi,sector,entyFe) 
         + ( p21_sub_convergence_rollback("2035",regi,sector,entyFe) - p21_tau_fe_sub("2025",regi,sector,entyFe) ) * ( (ttot.val - 2025) / (2035 - 2025) );
      p21_tau_fe_sub(ttot,regi,sector,entyFe)$(p21_sub_convergence_rollback("2035",regi,sector,entyFe) gt 0 AND ttot.val  gt 2035) = p21_sub_convergence_rollback("2035",regi,sector,entyFe);
    );
