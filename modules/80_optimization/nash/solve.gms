...
$endif.repeatNonOpt
  ...
  if(cm_nash_mode = 1, !! regions run in a sequence
    ...
    if(p80_repy_thisSolitr(all_regi,"modelstat") = 2,
...
loop(regi,
  ...
  if(p80_repy(regi,"modelstat") = 2 or p80_repy(regi,"modelstat") = 7,
    pm_SolNonInfes(regi) = 1;
  );
  if(p80_repy(regi,"modelstat") = 7, 
...
loop((regi,solveinfo80) $ p80_repy_thisSolitr(regi,solveinfo80),
  p80_repy_iteration(regi,solveinfo80,iteration) !! store sum of resusd for all sol_itrs
