...
*** Compute sum of foreign variables needed for regional optimisation (use the last known optimal value)
  ...
  loop(ttot $ (ttot.val >= 2005),
    pm_cumEff(ttot,regi,in)
      = sum(regi2 $ (not sameas(regi,regi2) and pm_SolNonInfes(regi2) = 1),
          pm_cesdata("2005",regi2,in,"eff") * vm_effGr.l(ttot,regi2,in)); !! TODO: case where regi2 is infeasible 

    pm_co2eqForeign(ttot,regi)           = sum(regi2 $ (not sameas(regi,regi2)), pm_co2eq0(ttot,regi2));
    pm_fuExtrForeign(ttot,regi,enty,rlf) = sum(regi2 $ (not sameas(regi,regi2)), vm_fuExtr.l(ttot,regi2,enty,rlf));
    pm_capCumForeign(ttot,regi,teLearn)  = sum(regi2 $ (not sameas(regi,regi2)), pm_capCum0(ttot,regi2,teLearn));
  );
