...
*** optimal solution, accept this solution as if it were optimal. 
  if (   p80_repy(regi,'modelstat') eq 7
         !! The 1E-4 are quite arbitrary. One should do more research on how the
	 !! solution differs over iteration when status 7 occurs. 
     AND p80_repy(regi,'objval') - p80_repyLastOptim(regi,'objval') lt - 1e-4
         !! there is no last iteration if this is the first iteration
     AND iteration.val gt 1,
       s80_bool = 0;
