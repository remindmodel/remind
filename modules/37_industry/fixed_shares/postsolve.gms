...
*** note: this does not split the energy carriers correctly across emissions markets following secInd37_emiMkt 
*** as the FE share of subsectors p37_shIndFE is only defined for total FE not FE per emissions market.
...
*** FE per subsector whose emissions can be captured (helper parameter for calculation of industry captured CO2 below)
o37_demFeIndSub_SecCC(ttot,regi,secInd37) 
  = sum( (se2fe(entySe, entyFe, te), sector2emiMkt("indst",emiMkt))$(macBaseInd37(entyFe,secInd37)), o37_demFeIndSub(ttot,regi,entySe,entyFe,secInd37,emiMkt));

*** industry captured CO2
pm_IndstCO2Captured(ttot,regi,entySe,entyFe,secInd37,emiMkt)$(entyFeCC37(entyFe) AND o37_demFeIndSub_SecCC(ttot,regi,secInd37) AND macBaseInd37(entyFe,secInd37)) 
  = sum( secInd37_2_emiInd37(secInd37,emiInd37)$(emiInd37_fuel(emiInd37)), vm_emiIndCCS.l(ttot,regi,emiInd37))
  * o37_demFeIndSub(ttot,regi,entySe,entyFe,secInd37,emiMkt) /
  o37_demFeIndSub_SecCC(ttot,regi,secInd37);
