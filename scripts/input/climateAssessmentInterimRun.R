...
cat(date(), "climateAssessmentInterimRun.R: Using ", gdxFile, file = cfg$logFile, append = TRUE)
runTimes <- c(runTimes, "preprocessing start" = Sys.time())

climateAssessmentInputData0 <- reportEmiForClimateAssessment(gdxFile) %>%
  as.quitte() %>%
...
if (cfg$isArchived) {
  ...
  tarfile <- archiveClimateAssessmentData(cfg$climateDir, cfg$archiveDir, extraFiles = extras)
  cat(
