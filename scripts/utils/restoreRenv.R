# restores current renv to the state described in selected lockfile e.g. from the archive, updates current renv.lock
local({
  ...
  if (length(argv) == 1) {
    ...
    lockfiles <- c(list.files(here::here("archive"), full.names = TRUE),
  ...
  if (!isTRUE(file.exists(lockfile))) {
    stop("lockfile '", lockfile, "' does not exist.")
  }
