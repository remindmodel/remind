...
plot_efficiency <- function(){
  ...
  FEdem <- readgdx(gdx, "vm_demFeForEs")[, value := value*TWa_2_EJ][ttot <= maxyr & ttot >= minyr]
  ES <- readgdx(gdx, "vm_cesIO")[tall <= maxyr & tall >= minyr]
  setnames(ES, c("tall", "value"), c("ttot", "es_demand"))

  FEdem <- rbindlist(list(
	FEdem[grepl("_pass_sm", all_esty), .(all_in="entrp_pass_sm", fe_demand=sum(value)), by=c("ttot", "all_regi")],
	FEdem[grepl("_pass_lo", all_esty), .(all_in="entrp_pass_lo", fe_demand=sum(value)), by=c("ttot", "all_regi")],
	FEdem[grepl("_frgt_sm", all_esty), .(all_in="entrp_frgt_sm", fe_demand=sum(value)), by=c("ttot", "all_regi")],
	FEdem[grepl("_frgt_lo", all_esty), .(all_in="entrp_frgt_lo", fe_demand=sum(value)), by=c("ttot", "all_regi")]))

  dt <- ES[FEdem, on=c("ttot", "all_regi", "all_in")]
  dt[, effc := fe_demand/es_demand] # MJ/pkm, MJ/tkm
  dt[, year := as.numeric(as.character(ttot))]

  ## passenger
  toplot <- dt[grepl("_pass_", all_in)]

  ppass <- ggplot(toplot)+
	geom_line(aes(x=year, y=effc, color=all_in, group=all_in)) +
	facet_wrap(~all_regi, nrow = 4, scales = "free")+
	labs(x = "", y = "energy intensity of pass. mobility [MJ/pkm]")+
	theme_minimal()+
	# scale_fill_manual("Vehicle Type",values = cols, breaks=legend_ord)+
	theme(axis.text.x = element_text(size = 8),
		  axis.text.y = element_text(size=8),
  ...
  ## freight
  toplot <- dt[grepl("_frgt_", all_in)]

  pfreight <- ggplot(toplot)+
	geom_line(aes(x=year, y=effc, color=all_in, group=all_in)) +
	facet_wrap(~all_regi, nrow = 4, scales = "free")+
	labs(x = "", y = "energy intensity of freight transportation [MJ/pkm]")+
	theme_minimal()+
	# scale_fill_manual("Vehicle Type",values = cols, breaks=legend_ord)+
	theme(axis.text.x = element_text(size = 8),
		  axis.text.y = element_text(size=8),
  ...
  return(list(ppass=ppass, pfreight=pfreight))
...
plotVint = function(vintcomp, newcomp, sharesVS1, loadFactor, annual_mileage){
...
p = plotVint(vintcomp, newcomp, sharesVS1 = shares$VS1_shares, loadFactor, annual_mileage)
