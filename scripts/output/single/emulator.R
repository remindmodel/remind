...
out <- gdx2::readGDX(gdx, name = "v30_priceshift", format = "first_found", field = "l") * 1000 / 31.536
...
myplot <- ggplot(data = dat, aes(x = Year, y = Value)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = plotstyle(levels(dat$Region))) +
  labs(y = "$/GJ") +
  facet_wrap(~Region)
...
out <- gdx2::readGDX(gdx, name = "v30_pricemult", format = "first_found", field = "l")
getNames(out) <- "dummy" # something has to be here, will be removed by collapseNames anyway
...
for (r in regions) {
  ...
  p <- ggplot(data = dat, aes(x = Year, y = Value)) +
    geom_line(aes(colour = Data3), size = 1) +
    labs(y = "EJ/yr") +
  ...
  if (r != "GLO") {
    ...
    p <- ggplot(data = dat, aes(x = Year, y = Value, colour = Data3)) +
      geom_line(size = 1) +
      labs(y = "$/GJ") +
...
var_pe <- c(
  "Costs|Biomass|MAgPIE (billion US$2017/yr)",
