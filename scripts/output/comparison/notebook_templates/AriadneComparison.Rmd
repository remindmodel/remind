...
## Set switches and filters for notebook
...
# historic reference year for percentage reductions
ref.year <- 1990


# should only a subset of scenario be plotted
plot.scen.subset <- F
# should ariadne-specific validation plots be created?
ariadne.val.plots <- T
# should historical data be shown in plots where available?
show.historical <- T
# should scenario specific plots be plotted
show.scen.plots <- T


# subset of scenarios considered
#plot.scen <- c( "Trend" ,"Bal","Elec_dom", "Elec_imp" ,"H2_dom", "H2_imp", "Synf_imp")
# name of scenario subset
scen.batch.name <- ""
#scen.batch.name <- "_CO2PriceKink"
...
df <- df.mod %>%
        filter( region %in% reg, period %in% plot.period,
                scenario %in% plot.scen)  %>%
        #remove duplicates if present
        distinct() %>% 
        order.levels(scenario = plot.scen)
...
df.emi.co2.ref <- df.hist %>% 
                filter(variable == "Emi|CO2", period == ref.year,
                       model == "CEDS") %>% 
...
p.emi.grossneg <- ggplot() +
                    ...
                    scale_y_continuous("CO2 Emissions (Mt CO2/yr)",
                                       sec.axis = sec_axis(~ (. - df.emi.co2.ref$Reference)  / df.emi.co2.ref$Reference * 100, 
                                                       name = paste0("Reduction\nrelative to ",ref.year," (%)"), breaks = seq(-120,0,20))) +
...
ggsave(plot = p.emi.grossneg, filename = paste0(plots.path, "/Emi_CO2GrossNeg.png"), width=25, height=10, units = "cm")
...
df.emi.ghg.ref <- df.hist %>% 
                filter(variable == "Emi|GHGtot", period == ref.year,
                       model == "CEDS") %>% 
                rename( Reference = value)
...
p.emighg.grossneg <- ggplot() +
                    ...
                    scale_y_continuous("GHG Emissions (Mt CO2eq/yr)",
                                       sec.axis = sec_axis(~ (. - df.emi.ghg.ref$Reference)  / df.emi.ghg.ref$Reference * 100, 
                                                       name = paste0("Reduction\nrelative to ",ref.year," (%)"), breaks = seq(-120,0,20))) +
                    scale_fill_manual(values = mip::plotstyle(vars), labels = mip::plotstyle(vars, out = "legend")) +
                    geom_hline(yintercept=0, size=0.8, linetype="dashed", alpha=0.7) +
...
ggsave(plot = p.emighg.grossneg, filename = paste0(plots.path, "/Emi_GHGGrossNeg.png"), width=25, height=10, units = "cm")
```
...
p.emi.rel <- ggplot() +
              ...
              facet_wrap(~variable) +
              scale_x_continuous("Year", breaks = c(1990,2000,plot.period)) +
...
co2.sink.source.label <- c("Carbon Management|Carbon Sources|Biomass|Pe2Se|+|Other w/ couple prod" = "Other Biomass w/ CC",
                      "Carbon Management|Carbon Sources|Biomass|Pe2Se|+|Hydrogen w/ couple prod" = "Biomass to H2 w/ CC",
                      "Carbon Management|Carbon Sources|Biomass|Pe2Se|+|Liquids w/ couple prod" = "Biomass to Liquids w/ CC",
                      ...
                      "Carbon Management|Carbon Sinks|CCU|+|Synthetic Liquids" = "Synthetic Liquids",
                      "Carbon Management|Carbon Sinks|CCU|+|Synthetic Gases" = "Synthetic Gases")



co2.sink.source.color <- c("Other Biomass w/ CC" = "seagreen1",
                          "Biomass to H2 w/ CC" = "lightseagreen",
                          "Biomass to Liquids w/ CC" = "seagreen4",
                          "Pe2Se Fossil CC" = "grey20",
                     "industry energy CC" = "red",
                      "industry process CC" = "purple",
                     "DAC" = "cyan",
...
p.co2.sink.source <- ggplot() +
                      geom_col(data=df.co2.sink.source,
                               ...
                               alpha=.6, width = 3) +
                      facet_wrap(~scenario, ncol = length(plot.scen)) +
...
tt = plotstyle.add( entity = "SE|Electricity|Wind|Onshore"  ,legend = "Wind Onshore", c("#337fff"), replace = T)
tt = plotstyle.add( entity = "SE|Electricity|Wind|Offshore"  ,legend = "Wind Offshore", c("#E41A1C"), replace = T)
...
### Liquid Synfuel: Sinks and Sources

```{r}
### plot with positive supply and negative demand bars
synliqsup.focus.vars <- c("SE|Liquids|Hydrogen",
                      "SE|Liquids|Hydrogen|Net Imports")

synliqdem.focus.vars <- c("FE|Industry|Liquids|+|Hydrogen",
                      "FE|Buildings|Liquids|+|Hydrogen",
                      "FE|Transport|Liquids|+|Hydrogen")

synliq.sink.source.label <- c("SE|Liquids|Hydrogen" = "Liquid Synfuel Production",
                      "SE|Liquids|Hydrogen|Net Imports" = "Liquid Synfuel Import",
                      "FE|Industry|Liquids|+|Hydrogen" = "Synfuel Use Industry",
                      "FE|Buildings|Liquids|+|Hydrogen" = "Synfuel Use Buildings",
                      "FE|Transport|Liquids|+|Hydrogen" = "Synfuel Use Transport")

synliq.sink.source.color <- c("Liquid Synfuel Production" = "mediumpurple1",
                          "Liquid Synfuel Import" = "darkslateblue",
                          "Synfuel Use Industry" = "red",
                          "Synfuel Use Buildings" = "grey80",
                          "Synfuel Use Transport" = "blue")

vars.order <- c("Liquid Synfuel Production",
                "Liquid Synfuel Import",
                "Synfuel Use Industry",
                "Synfuel Use Buildings",
                "Synfuel Use Transport")



df.synliq.sink.source <- df %>%
                       filter(variable %in% synliqsup.focus.vars) %>%
                       # bind demand variables with negative sign
                        rbind( df %>%
                              filter(variable %in% synliqdem.focus.vars) %>%
                              # change sign an remove "SE|Electricity|used for"
                              mutate( value = - value)) %>%
                        revalue.levels(variable = synliq.sink.source.label) %>%
                        order.levels(variable = vars.order) %>%
                        filter(period %in% plot.period)



p.synliq.sink.source <- ggplot() +
                      geom_col(data=df.synliq.sink.source,
                               aes(period, value, fill=variable),
                               alpha=.6, width = 3) +
                      facet_wrap(~scenario, ncol = length(plot.scen)) +
                      scale_y_continuous("Liquid Synfuel (EJ/yr)") +
                      scale_fill_manual(values = synliq.sink.source.color) +
                      theme_bw() +
                      theme( axis.text.x = element_text(angle=90, vjust = 0.5),
                        legend.position = "bottom",
                        strip.background = element_blank(),
...
p.synliq.sink.source


ggsave(plot = p.synliq.sink.source,
       filename =  paste0(plots.path, "/SE_synliq_SinkSource.png"),
       width=25, height=10, units="cm")
```
...
### Capacity Additions for Wind and Solar


```{r}
vars.capadd.VRE <- c("New Cap|Electricity|Wind",
                     "New Cap|Electricity|Solar|PV")


# define colors and labels
tt = plotstyle.add(c("New Cap|Electricity|Wind")   ,c("Wind"),  c( "deepskyblue1"),replace=T)
tt = plotstyle.add(c("New Cap|Electricity|Solar|PV")   ,c("Solar PV"),  c( "yellow"),replace=T)


df.capadd.VRE <- df %>% 
                  filter(variable %in% vars.capadd.VRE, period >= 2020,period <= 2040) %>% 
                  mutate(period = as.factor(period)) %>% 
                  revalue.levels(variable = c("New Cap|Electricity|Wind" = "Wind", "New Cap|Electricity|Solar|PV" = "Solar PV"))
                  

df.cappadd.VRE.hist <- df.hist %>% 
                        filter(variable %in% c("Cap|Electricity|Wind", "Cap|Electricity|Solar|PV"),
                               period %in% seq(2010,2020,1), model == "IRENA") %>%
                        # average and maximum capacity additions over 2011-2020
                        group_by(model, scenario, region, variable, unit) %>% 
                        mutate( CapAdd = value - lag(value)) %>% 
                        mutate(Avg = mean(CapAdd, na.rm = T), Max = max(CapAdd, na.rm = T)) %>% 
                        ungroup() %>% 
                        # relabel as capacity additions
                        revalue.levels(variable = c("Cap|Electricity|Wind" = "Wind",
                                                    "Cap|Electricity|Solar|PV" = "Solar PV")) 


df.cappadd.VRE.hist.plot <- df.cappadd.VRE.hist %>% 
                        # put label for average and maximum into scenario dimensions
                        select(-scenario) %>% 
                        gather(scenario, value, Avg, Max) %>%  
                        # choose one arbitrary period to select average and maximum
                        filter(period == 2010) %>% 
                        mutate( period = scenario) %>% 
                        revalue.levels(period = c("Avg" = "2011-20 Avg.", "Max" = "2011-20 Max.")) %>% 
                        mutate( period = as.factor(period))
                      


p.capadd.VRE <- ggplot() +
                  geom_col(data=df.capadd.VRE, aes(period, value, fill=scenario), alpha=0.7, position = "dodge") +
                  geom_col(data=df.cappadd.VRE.hist.plot, aes(period, value, fill = model), alpha=0.7, position = "dodge") +
                  facet_wrap(~variable, nrow = length(vars.capadd.VRE)) +
                  scale_y_continuous("Capacity Additions (GW/yr)") +
                  theme_bw() +
                  theme( axis.text.x = element_text(angle=90, vjust = 0.5),
...
ggsave(plot = p.capadd.VRE,
       filename =  paste0(plots.path, "/SE_CapAdd_VRE.png"),
       width=25, height=10, units="cm")
...
vars = c("Price|Secondary Energy|Electricity|Moving Avg",
         ...
         "Price|Secondary Energy|Liquids|Biomass|Moving Avg",
         "Price|Secondary Energy|Liquids|Fossil|Moving Avg",
         "Price|Secondary Energy|Liquids|Hydrogen|Moving Avg",
         "Price|Secondary Energy|Gases|Biomass|Moving Avg",
          "Price|Secondary Energy|Liquids|Biomass|Moving Avg",
...
#save plots
ggsave(plot = p.SE.price,filename = paste0(plots.path, "/Prices_SE", ".png"),width=20, height=10, units = "cm")
...
if ("Emi|CO2|Transport|Shipping|Demand" %in% getVars(df)) { 
...
if ("FE|Non-energy Use|Industry|+|Liquids"  %in% getVars(df)) {
...
p.ind.FE.detail <- ggplot() +
                        geom_col(data=df.ind.FE.detail, aes(period, value, fill=fuel), alpha=0.7) +
...
if (show.historical) {
  ...
  vars.ind.FE.hist.map <- c("FE|Industry|Cement" = "Cement","FE|Industry|Steel" = "Steel", 
                            "FE|Industry|Chemicals" = "Chemicals")
  
  

  df.ind.FE.hist <- df.hist %>%
                    filter(variable %in% names(vars.ind.FE.hist.map), period <= max(plot.period), model == "JRC") %>% 
                    select(-scenario) %>% 
                    mutate(sector = variable) %>% 
                    revalue.levels(sector = vars.ind.FE.hist.map) %>% 
                    full_join( df.ind.FE.detail %>%
                                 filter(period == 2015, fuel == "Liquids") %>%
                                 select(sector, scenario)
                                 ) %>% 
                    filter( !is.na(value))
  
  p.ind.FE.detail <- p.ind.FE.detail +
                      geom_line(data=df.ind.FE.hist, aes(period, value, color=model), size=1.2, alpha=0.7) 
...
### config
vars.ind.prod <- c("Production|Industry|Steel","Production|Industry|Steel|Primary","Production|Industry|Steel|Secondary",  "Production|Industry|Cement")


ind.prod.label <- c("Production|Industry|Steel" = "Steel",
                    "Production|Industry|Steel|Primary" = "Primary Steel",
...
if (show.historical) {
  ...
  vars.ind.prod.hist.map <- c("Production|Industry|Cement" = "Cement","Production|Industry|Steel|Primary" = "Primary Steel", 
                            "Production|Industry|Steel|Secondary" = "Secondary Steel", "Production|Industry|Steel" = "Steel")
  
  
  
  

  df.ind.prod.hist <- df.hist %>%
                    filter(variable %in% names(vars.ind.prod.hist.map), period <= max(plot.period), model == "JRC") %>% 
                    select(-scenario) %>% 
                    revalue.levels(variable = vars.ind.prod.hist.map) %>% 
                    # expand dimensions to have both variable and scenario dimensions for faceting
                    full_join( df.ind.prod %>%
                                 filter(period == 2015) %>%
                                 select(variable, scenario)
                                 ) %>% 
                    filter( !is.na(value))
  
  p.ind.prod <- p.ind.prod +
                      geom_line(data=df.ind.prod.hist, aes(period, value, color=model), size=1.2, alpha=0.7)
  
  
  
  vars.ind.value.hist.map <- c("Value Added|Industry|Chemicals" = "Chemicals")
  
  
  df.ind.value.hist <- df.hist %>%
                    filter(variable %in% names(vars.ind.value.hist.map), period <= max(plot.period), model == "JRC") %>% 
                    select(-scenario) %>% 
                    revalue.levels(variable = vars.ind.value.hist.map) %>% 
                    full_join( df.ind.value %>%
                                 filter(period == 2015) %>%
                                 select(variable, scenario)
                                 ) %>% 
                    filter( !is.na(value))
  
  p.ind.value <- p.ind.value +
                      geom_line(data=df.ind.value.hist, aes(period, value, color=model), size=1.2, alpha=0.7)
...
if (ariadne.val.plots) {
    ...
    cumul.period <- seq(2020,2045,5)
  
    # emissions variables to cumulate
    cumul.vars <- c("Emi|CO2",
                "Emi|CO2|+|Energy")
    
    cumul.target <- data.frame(variable = c("Emi|CO2","Emi|CO2|+|Energy"), value = c(8.2,7.2))


    df.cumul <- df %>%
              filter(variable %in% cumul.vars, period %in% cumul.period) %>%
              right_join(remind_timesteps %>%
                           filter(period %in% cumul.period)) %>%
              # adjust weights to how budget calculation is done in regipol module (0.5 factor for edge years of budget)
              mutate( weight = ifelse(period == min(cumul.period) | period == max(cumul.period), 0.5,1)) %>%
    ...
    p.emi.cumul <- ggplot(df.cumul, aes(year, cumsum.value)) +
                  ...
                  theme( axis.text.x = element_text(angle = 90),
                         strip.background = element_blank()) +
                  scale_y_continuous("2020-2045 Cumulative Emissions (Gt CO2)")
...
if (show.scen.plots) {
  ...
  elec.h2.col <- c("Other Energy" = "grey95",
                   "Electricity|Min" =  "#ffb200",
                   "Electricity|Max" =  "#ffb200",
                   "Hydrogen|Max" = "#66cccc",
                   "Hydrogen|Min" = "#66cccc")
  
  
  h2.elec.alpha <- c("Other Energy" = 0.4,
                   "Electricity|Min" =  0.7,
                   "Electricity|Max" =  0.4,
                   "Hydrogen|Max" = 0.4,
                   "Hydrogen|Min" = 0.7)
  ...
  df.elec.h2 <- df %>% 
                  calc_addVariable("`Electricity`" = "`FE|+|Electricity` / `FE` * 100",
                                   "`Hydrogen`" = "`FE|+|Hydrogen` / `FE` * 100", only.new = T, units = "%") %>% 
                  filter( scenario %in% c(scen.bot, scen.top)) %>%
                  rename( encar = variable) %>% 
                  # rename scenarios to Max and Min
                  mutate(MinMax.Scenario = ifelse( (encar == "Electricity" & scenario == "Elec_dom") | (encar == "Hydrogen" & scenario == "H2_dom"), 
                                            "Max", "Min")) %>%  
                  mutate( variable = paste0(encar,"|",MinMax.Scenario)) %>% 
                  select(region, period, variable, value) %>% 
                  spread(variable, value) %>% 
                  mutate(`Other Energy` = 100-`Electricity|Max`-`Hydrogen|Max`,
                         `Electricity|Max` = `Electricity|Max`-`Electricity|Min`,
                         `Hydrogen|Max` = `Hydrogen|Max`-`Hydrogen|Min`) %>% 
                  mutate(`Other Energy` = ifelse(`Other Energy` < 0, 0, `Other Energy`)) %>% 
                  gather(variable, value, `Electricity|Min`, `Electricity|Max`, `Hydrogen|Min`, `Hydrogen|Max`, `Other Energy`) %>% 
                  mutate( energy = word(variable, 1, sep="\\|"), scenario = word(variable, 2, sep="\\|")) %>% 
                  mutate( scenario = ifelse(energy == "Other Energy", "Max", scenario)) %>% 
                  order.levels(variable = c("Hydrogen|Min","Hydrogen|Max","Other Energy","Electricity|Max","Electricity|Min"))
  ...
  p.elec.h2 <- ggplot() +
                geom_area(data = df.elec.h2, aes(period, value, fill=variable, alpha=variable)) +
                scale_fill_manual(values= elec.h2.col ) +
                scale_alpha_manual(values =h2.elec.alpha) +
                scale_y_continuous("Share in Final Energy (%)", breaks = seq(0,100,10)) +
                scale_x_continuous("Year") +
                  theme_bw() +
  ...
  ggsave(plot = p.elec.h2, filename =  paste0(plots.path, "/H2Elec_Area.png"),width=20, height=15, units="cm")
...
vars <- c("FE|Industry|+|Electricity",
          "FE|Industry|+|Hydrogen",
          "FE|Industry|+|Liquids",
          "FE|Industry|+|Gases")

col.vars <- c("Electricity" = "#ffb200",
              "Hydrogen" = "#66cccc",
              "Liquids" = "darkblue",
              "Gases" = "gray60")
...
df.IndShare <- df %>% 
                filter(period %in% c(2015,2030,2040,2050,2060)) %>% 
                calc_addVariable("Electricity" = "`FE|Industry|+|Electricity` / `FE|++|Industry` * 100",
                                 "Hydrogen" = "`FE|Industry|+|Hydrogen` / `FE|++|Industry` * 100",
                                 "Liquids" = "`FE|Industry|+|Liquids` / `FE|++|Industry` * 100",
                                 "Gases" = "`FE|Industry|+|Gases` / `FE|++|Industry` * 100",
                                    only.new = T, units = "%")


p.IndShare <- ggplot() +
                geom_col(data=df.IndShare, aes(period, value, fill=variable), alpha=0.6) +
                geom_text(data=df.IndShare, aes(period, value, label = as.character(round(value))), size=3) +
                facet_wrap(variable~scenario, ncol = length(getScenarios(df))) +
                scale_y_continuous("Industry: \nShare in Final Energy (%)") +
                scale_x_continuous("Years") +
                scale_fill_manual(values=col.vars) +
                theme_bw() +
...
ggsave(plot = p.IndShare, filename =  paste0(plots.path, "/Industry_FEShare.png"),width=20, height=15, units="cm")
```
...
vars <- c("FE|Buildings|Heating|Electricity|Resistance",
...
col.vars <- c("Resistive electricity" = "#ffb200",
              "Heat pump electricity" = "darkorange",
              "Hydrogen" = "#66cccc",
              "Gases" = "gray90",
...
df.BuildShare <- df %>% 
                filter(period %in% c(2015,2030,2040,2050,2060)) %>% 
                calc_addVariable("Resistive electricity" = "`FE|Buildings|Heating|Electricity|Resistance` / `FE|Buildings|Heating` * 100",
                                 "Heat pump electricity" = "`FE|Buildings|Heating|Electricity|Heat pumps` / `FE|Buildings|Heating` * 100",
                                 "Hydrogen" = "`FE|Buildings|Heating|Hydrogen` / `FE|Buildings|Heating` * 100",
                                 "Gases" = "`FE|Buildings|Heating|Gases` / `FE|Buildings|Heating` * 100",
                                 "District Heating" = "`FE|Buildings|Heating|District Heating` / `FE|Buildings|Heating` * 100",
                                    only.new = T, units = "%") %>% 
                order.levels(variable = c(  "Heat pump electricity", "Resistive electricity" , "District Heating","Gases",   "Hydrogen"))


p.BuildShare <- ggplot() +
                geom_col(data=df.BuildShare, aes(period, value, fill=variable), alpha=0.6) +
                geom_text(data=df.BuildShare, aes(period, value, label = as.character(round(value))), size=3) +
                facet_wrap(variable~scenario, ncol = length(getScenarios(df))) +
                scale_y_continuous("Buildings Heating: \n Share in Final Energy Heating (%)") +
                scale_x_continuous("Years") +
               # scale_fill_manual(values=col.vars) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 90),legend.position = "right")
...
ggsave(plot = p.BuildShare, filename =  paste0(plots.path, "/Build_FEShare.png"),width=20, height=15, units="cm")
```
### Transport
...
## LDV
vars <- c("FE|Transport|Pass|Road|LDV|Electricity",
          "FE|Transport|Pass|Road|LDV|Hydrogen",
          "FE|Transport|Pass|Road|LDV|Liquids")
...
df.LDVShare <- df %>% 
                filter(period %in% c(2015,2030,2040,2050,2060)) %>% 
                calc_addVariable("Electricity" = "`FE|Transport|Pass|Road|LDV|Electricity` / `FE|Transport|Pass|Road|LDV` * 100",
                                 "Hydrogen" = "`FE|Transport|Pass|Road|LDV|Hydrogen` / `FE|Transport|Pass|Road|LDV` * 100",
                                 "Liquids" = "`FE|Transport|Pass|Road|LDV|Liquids` / `FE|Transport|Pass|Road|LDV` * 100",
                                    only.new = T, units = "%")


p.LDVShare <- ggplot() +
                geom_col(data=df.LDVShare, aes(period, value, fill=variable), alpha=0.6) +
                geom_text(data=df.LDVShare, aes(period, value, label = as.character(round(value))), size=3) +
                facet_wrap(variable~scenario, ncol = length(getScenarios(df))) +
                scale_y_continuous("LDV Transport: \n Share in Final Energy (%)") +
                scale_x_continuous("Years") +
               # scale_fill_manual(values=col.vars) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 90),legend.position = "right")
...
ggsave(plot = p.LDVShare, filename =  paste0(plots.path, "/Transport_LDV_FEShare.png"),width=20, height=15, units="cm")
...
## Trucks
vars <- c("FE|Transport|Freight|Road|Electricity",
          "FE|Transport|Freight|Road|Hydrogen",
          "FE|Transport|Freight|Road|Liquids")
...
df.HDVShare <- df %>% 
                filter(period %in% c(2015,2030,2040,2050,2060)) %>% 
                calc_addVariable("Electricity" = "`FE|Transport|Freight|Road|Electricity` / `FE|Transport|Freight|Road` * 100",
                                 "Hydrogen" = "`FE|Transport|Freight|Road|Hydrogen` / `FE|Transport|Freight|Road` * 100",
                                 "Liquids" = "`FE|Transport|Freight|Road|Liquids` / `FE|Transport|Freight|Road` * 100",
                                    only.new = T, units = "%")


p.HDVShare <- ggplot() +
                geom_col(data=df.HDVShare, aes(period, value, fill=variable), alpha=0.6) +
                geom_text(data=df.HDVShare, aes(period, value, label = as.character(round(value))), size=3) +
                facet_wrap(variable~scenario, ncol = length(getScenarios(df))) +
                scale_y_continuous("Truck Transport: \n Share in Final Energy (%)") +
                scale_x_continuous("Years") +
               # scale_fill_manual(values=col.vars) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 90),legend.position = "right")
...
ggsave(plot = p.HDVShare, filename =  paste0(plots.path, "/Transport_HDV_FEShare.png"),width=20, height=15, units="cm")
