...
vintcomparisonpf = function(dt){
  ...
  dt_glob[, region := "GLO"]
  dt = rbind(dt, dt_glob)
  cols2use=as.character(unique(dt[,technology]))
  plot = ggplot()+
    geom_bar(data = dt,
   ...
   plot1 = ggplot()+
    geom_bar(data = dt[variable == "newdem"],
             aes(x=scenario, y=value, group=interaction(variable, technology),
                 fill = technology, width=.75), alpha = 0.5, position="stack", stat = "identity", width = 0.5)+
    geom_bar(data = dt[variable == "newdem"],
             aes(x=scenario, y=value, group=interaction(variable, technology),
    ...
    guides(linetype=FALSE,
           fill=guide_legend(reverse=FALSE, title="Transport mode"))+
    scale_fill_manual(values = cols[match(cols2use, names(cols))]))+
    labs(y = "[million Veh]", x="", title = "LDV new sales fleet")

return(list(plot=plot, plot1=plot1))
...
salescompf = function(dt){
  ...
  plot = ggplot()+
    ...
    theme2use +
    scale_fill_manual("Technology", values = cols[match(cols2use, names(cols))])+
...
EJroadpf = function(dt){
  ...
  plotLDV = ggplot()+
    ...
    scale_fill_manual("Technology", values = cols[match(cols2use, names(cols))], breaks=legend_ord)+
    expand_limits(y = c(0,1))+
...
EJmodepf = function(dt){
  ...
  dt = dt[year >= 2010]
  cols2use = as.character(unique(dt[, vehicle_type_plot]))
  plot = ggplot()+
    ...
    facet_grid(region~scenario, scales = "free")+
    scale_fill_manual("Vehicle Type",values = cols[match(cols2use, names(cols))], breaks=legend_ord)+
    expand_limits(y = c(0,1))+
    scale_x_continuous(breaks = c(2020,2030,2050, 2100)) +
    theme2use
...
ESmodecappf = function(dt){
  ...
  cols2use = as.character(unique(dt[mode == "freight", vehicle_type_plot]))
  plot_frgt = ggplot()+
  ...
  cols2use = as.character(unique(dt[mode == "pass", vehicle_type_plot]))
  plot_pass = ggplot()+
...
EJfuelsMod_pf = function(dt_all){
  ...
  cols2use = as.character(unique(dt_all_b[, techmode]))
  plot_b_lo = ggplot()+
    ...
    facet_grid(region~scenario, scales = "free")+
    scale_fill_manual("Transport mode",values = cols[match(cols2use, names(cols))], breaks = legend_ord)+
    expand_limits(y = c(0,1))+
    scale_x_continuous(breaks = c(2020, 2030,2050, 2100)) +
    theme2use

  cols2use = as.character(unique(dt_all_nob[, techmode]))
  plot_nb_lo = ggplot()+
...
EJfuels_pf = function(dt_p, dt_f){
  ...
  cols2use = as.character(unique(dt_p[sector == "trn_pass", subtech]))
  plotp = ggplot()+
    ...
    scale_x_continuous(breaks = c(2020, 2030,2050, 2100)) +
    theme2use
  cols2use = as.character(unique(dt_p[sector == "trn_aviation_intl", subtech]))
  plotp_LO = ggplot()+
  ...
  cols2use = as.character(unique(dt_p[sector == "trn_shipping_intl", subtech]))
  plotf_lo = ggplot()+
  ...
  cols2use = as.character(unique(dt_p[sector == "trn_freight", subtech]))
  plotf_sm = ggplot()+
  ...
  cols2use = as.character(unique(dt_all_b[, subtech]))
  plot_all = ggplot()+
  ...
  cols2use = as.character(unique(dt_all_nob[, subtech]))
