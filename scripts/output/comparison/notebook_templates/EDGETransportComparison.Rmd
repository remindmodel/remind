...
legend_ord_costs <- c("Inconvenience cost", "Risk", "Charging", "Model availability", "Range anxiety", "Refuel availability", "Fuel", "Policy induced inconvenience", "Purchase", "Operating and maintenance")
...
plot = list(plot1, plot2)

return(plot)
}

costspf(costs_all, rp = regionplot)
```
...
p = ggplot()+
  geom_line(data = pref, aes(x = year, y = value, group = technology, color = technology))+
  facet_wrap(~scenario)+
  theme_minimal()+
  ...
  scale_color_manual("Technology", values = cols)
