...
vintcomparisontotpf = function(dt, rp){
    ...
    dt[, type := ifelse(grepl("Wise",scenario), "WiseWays", "Normal"), by = c("region", "year", "scenario")]
...
elecdem_pf = function(dt, rp){
 ...
 dt[, scenario := gsub("Wise", "", scenario)]
  ...
  plot_all = ggplot()+
    geom_line(data = dt, aes(x = year, y = value, group = scenario, color = scenario, linetype = type))+
   ...
   plot_regi = ggplot()+
    geom_line(data = dt[region == rp & year <= 2050], aes(x = year, y = value, group = scenario, color = scenario, linetype = type))+
    labs(x = "", y = "Secondary electricity [EJ]")+
    theme_minimal()+
    theme(axis.text.x = element_text(angle = 90,  size = 14, vjust=0.5, hjust=1),
          axis.text.y = element_text(size = 14),
...
elecdem_pf(elecdem_all, rp = regionplot)
```
...
CO2km_int_regi_newsalespf = function(dt, rp){
  ...
  dt[, type := ifelse(grepl("Wise",scenario), "WiseWays", "Normal"), by = c("region", "year", "scenario")]
   ...
   plot = ggplot()+
    geom_line(data = dt[year >= 2020 & year <= 2050 & region == rp], aes(x = year, y = gCO2_km_ave, group = scenario, color = scenario, linetype = type))+
