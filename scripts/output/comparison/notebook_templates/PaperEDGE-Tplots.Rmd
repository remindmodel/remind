...
legend_ord_modes <- c("Freight Rail", "Truck", "Shipping", "International Shipping", "Domestic Shipping",  "Trucks", "Motorbikes", "Small Cars", "Large Cars", "Van",
                ...
                "Freight", "LDV", "Pass non LDV", "Freight (Inland)", "Inland Freight", "Pass non LDV (Domestic)", "Inland non-LDV", "Non motorized")
...
kayapf = function(dtES, dtFE, dtEmi, rp, mapping){
    ...
    dt = merge(dtFE, dtEScap[, vehicle_type_plot := NULL], by = c("region", "year", "scenario"))
...
costspf = function(dt, rp, mapping_scens){
    dt = dt[region == rp & scenario %in% c("Budg1100_ConvCase", "Budg1100_ElecEra","Budg1100_HydrHype", "NPi_ConvCase") & year %in% c(2015, 2030, 2050)]
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_tot[year == 2015 & scen_name == "NPi_Conv"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name, alpha = alph), position = "stack", stat = "identity")+
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_inc[year == 2015 & scen_name == "NPi_Conv"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name), position = "stack", stat = "identity")+
...
p = salescompf(salescomp_all, rp = regionplot)

p

aspect_ratio <- 2
height <- 8
ggsave("sales_composition.png", p, dpi=500, height = height , width = height * aspect_ratio)
```
...
p = EJmodepf(EJmode_all, rp = regionplot)
aspect_ratio <- 2
height <- 2
ggsave("FEbymode.png", p, dpi=500, height = height , width = height * aspect_ratio)
```
...
HDV_PEpf = function(dt, rp, mapping){
   ...
   p = ggplot()+
        ...
        facet_wrap(~scen_name, nrow = 1)+
  theme2use +
  labs(y = "Primary energy [EJ]", x="", title = "HDVs")+
  scale_fill_manual("Source", values = cols)+
  scale_x_continuous(breaks = c(2020, 2030, 2050))+
  theme(strip.text = element_text(size = 12))
...
p = emitail_pf(emitail_all, regionplot)
...
p = EJmodepf(EJmode_all, regionplot)
aspect_ratio <- 3
height <- 4
...
EJfuelsMode_pf = function(dt, rp){
  ...
  dt_all = dt[,.(demand_EJ = sum(demand_EJ)), by =.(region, year, subtech)]
  plotAllT = ggplot()+
...
p = EJfuels_pf(dt = EJfuelsMode_all, rp = regionplot)
aspect_ratio <- 3
height <- 4
ggsave("FEfuelMode.png", p, dpi=500, height = height , width = height * aspect_ratio)
```
