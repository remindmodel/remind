...
costs_detailed_pf = function(dt, rp, mapping_scens){
    ...
    dt_tot[, alph := ifelse(type == "inc", 0.6, 0.9)]
    ...
    plot = ggplot()+
      ...
      guides(alpha=FALSE,
             linetype=FALSE,
             fill=guide_legend(reverse=FALSE, title="Cost component"))+
      theme(axis.title.y = element_blank(),
            axis.text.y = element_blank())
...
aspect_ratio <- 2
height <- 8
ggsave("pvintArea.png", p, dpi=400, height = height , width = height * aspect_ratio)
```
...
aspect_ratio <- 5.05
height <- 4
ggsave("emiDemand_mode.png", p, dpi=500, height = height , width = height * aspect_ratio)
```
