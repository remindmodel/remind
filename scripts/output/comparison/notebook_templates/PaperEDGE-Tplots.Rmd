...
costspf = function(dt, rp, mapping_scens){
    dt = dt[region == rp & scenario %in% c("Budg1100_ConvCase", "Budg1100_ElecEra","Budg1100_HydrHype", "NPi") & year %in% c(2015, 2030, 2050)]
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_tot[year == 2015 & scen_name == "NPi_IC"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name, alpha = alph), position = "stack", stat = "identity")+
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_inc[year == 2015 & scen_name == "NPi_IC"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name), position = "stack", stat = "identity")+
...
aspect_ratio <- 2.5
height <- 5
ggsave("LDV_PE.png", p_onlySyn, dpi=500, height = height , width = height * aspect_ratio)
