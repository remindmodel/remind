...
kayapf = function(dtES, dtFE, dtEmi, rp, mapping){
    ...
    dtES = dtES[,.(demand_F = sum(demand_F), pop = mean(pop)), by = .(region, year, scenario)]
    dtES[, EScap := demand_F/pop]
    ...
    dtEmi = dtEmi[region == rp & year >= 2015]
    ...
    dt = melt(dt[,c("region", "year", "scenario", "EScapindex", "FEcapindex", "eintindex")], id.vars = c("region", "year", "scenario"))
    
    dt = merge(dt, mapping, by = "scenario")
    dt = dt[scen_name %in% c("Budg_IC", "Budg_IC-LS", "Budg_BE", "Budg_BE-LS", "Budg_FC", "Budg_ICsyn", "NPi_IC")]
...
costspf = function(dt, rp, mapping_scens){
    dt = dt[region == rp & scenario %in% c("Budg1100_ConvCase", "Budg1100_ElecEra","Budg1100_HydrHype", "NPi") & year %in% c(2020, 2030, 2050)]
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_tot[year == 2020 & scen_name == "NPi_IC"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name, alpha = alph), position = "stack", stat = "identity")+
    ...
    plot_proj = ggplot()+
      geom_bar(data = dt_tot[year %in% c(2030, 2050) & scen_name != "NPi_IC"], aes(x = technology, y = cost, group = name, fill = name, alpha = alph), position = "stack", stat = "identity") +
      facet_wrap(~year+scen_name, nrow = 1)+
      theme2use +
      ylim(0,1.01*unique(dt_tot[, maxval]))+
      scale_fill_manual(values = cols)+
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_inc[year == 2020 & scen_name == "NPi_IC"][,scen_name := "Historical"], aes(x = technology, y = cost, group = name, fill = name), position = "stack", stat = "identity")+
    ...
    plot_proj = ggplot()+
      geom_bar(data = dt_inc[year %in% c(2030, 2050)& scen_name != "NPi_IC"], aes(x = technology, y = cost, group = name, fill = name), position = "stack", stat = "identity") +
      facet_wrap(~year+scen_name, nrow = 1)+
      theme2use +
      ylim(0,1.01*unique(dt_inc[, maxval]))+
      scale_fill_manual(values = cols)+
...
vintcomparisonArea_regi_pf = function(dt, rp, mapping){
  ...
  dt = merge(dt, mapping)
  dt = dt[scen_name %in% c("NPi_IC", "Budg_IC", "Budg_BE", "Budg_FC", "Budg_ICsyn", "Budg_IC-LS", "Budg_BE-LS")]
  dt[, scen_name := factor(scen_name, levels = c("NPi_IC", "Budg_IC", "Budg_BE", "Budg_FC", "Budg_ICsyn", "Budg_IC-LS", "Budg_BE-LS"))]
  ...
  pbar = ggplot()+
   ...
   plot = plot_grid(parea, pbar, align = "v", ncol = 1, rel_heights  = c(0.5, 0.5))  
   return(plot)
}
...
p = ggplot()+
  geom_line(data = pref[scen_name %in% c("Budg_BE", "Budg_FC", "Budg_IC")], aes(x = year, y = value, group = interaction(technology, vehicle_type), color = technology, linetype = vehicle_type))+
...
trspPEpf = function(dt, rp, mapping){
   ...
   dt[, scen_name := factor(scen_name, levels = c("NPi_IC", "Budg_IC", "Budg_ICsyn", "Budg_FC", "Budg_BE", "Budg_IC-LS", "Budg_BE-LS"))]
   p = ggplot()+
        geom_area(data = dt, aes(x = year, y = value, fill = fuel, group = fuel))+
        facet_wrap(~scen_name, nrow = 1)+
  theme2use +
  labs(y = "Secondary energy [EJ]", x="")+
