...
if (! exists("project")) {
  project <- gms::chooseFromList(c("ECEMF", "ENGAGE_4p5", "ELEVATE", "NAVIGATE", "NGFS_v4", "SHAPE"), type = "project", multiple = FALSE,
...
withCallingHandlers({ # piping messages to logFile
  ...
  mifdata$scenario <- gsub("GP_CurPol_T45", "ELV_CurPol_T44",
                      gsub("GP_NDC2030_T45", "ELV_NDC2030_T44",
                           mifdata$scenario
                      ))
