...
modelSummary <- function(folder = ".", gams_runtime = NULL) {
  ...
  if (!file.exists(file.path(folder, "full.gms"))) {
    ...
    if (!file.exists(file.path(folder, "full.lst")) || !file.exists(file.path(folder, "full.log"))) {
      ...
      if (!file.exists(file.path(folder, "non_optimal.gdx"))) {
        ...
        message(
          "  non_optimal.gdx exists, because iteration ", max_iter_no, " did not find a locally optimal solution. ",
      ...
      if (!file.exists(file.path(folder, "fulldata.gdx"))) {
        ...
        message(
          "  fulldata.gdx exists, because iteration ", max_iter_fd, " was successful. ",
      ...
      if (length(modelstat) > 0) {
        ...
        message(
          "  Modelstat after ", as.numeric(names(modelstat)), " iterations: ", modelstat,
  ...
  if (file.exists(file.path(folder, "fulldata.gdx")) && identical(cfg$gms$optimization, "nash") &&
    ...
    failedmarkets <- quitte::as.quitte(readGDX(gdx = file.path(folder, "fulldata.gdx"), "p80_messageFailedMarket"))
    ...
    failedtaxes <- quitte::as.quitte(readGDX(gdx = file.path(folder, "fulldata.gdx"), "p80_convNashTaxrev_iter"))
